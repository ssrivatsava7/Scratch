...
class PlaylistDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Get playlist name from route parameters or arguments
    final name = Get.parameters['name'] ?? Get.arguments?['name'] ?? '';
    
    print('=== PLAYLIST DETAIL SCREEN ===');
    ...
    return Obx(() {
      final playlist = Controllers.playlist.playlists
          .firstWhere((p) => p['name'] == name, orElse: () => {});
      ...
      // Get items from playlist (handle both 'items' and 'tracks' keys)
  ...
  void _showRemoveDialog(String playlistName, Map<String, dynamic> item) {
    print('Remove clicked for: $playlistName - ${item["title"]}');
    ...
    Get.dialog(
      AlertDialog(
        ...
        content: Text(
          'Remove "${item['title']}" from "$playlistName"?',
